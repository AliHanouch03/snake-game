...
class Snake:
    ...
    def add_segment(self, position): # creates a new segment and places it at 'position'
        new_segment = Turtle(shape='square')
        new_segment.color('white')
        new_segment.penup() # not write the path of the snake on the screen
        new_segment.goto(position) # fixes the bug: the square shows at the center at each food-snake collision
        self.segments.append(new_segment) # append it to the list of segments

    def extend(self): # adds a new segment to the snake
        # add_segment is triggered with the argument position of the last segment of the snake
